...
// Posiciones de las lamparas (ajusta segun tu escena)
glm::vec3 lampPositions[] = {
	glm::vec3(8.0f, 3.5f, -10.0f),  // sobre Stand1
	glm::vec3(8.0f, 3.5f,   3.0f),  // sobre Stand3
	glm::vec3(18.1f, 3.5f, -20.0f),  // sobre Stand4
	glm::vec3(22.0f, 3.5f,  -1.0f),  // cerca de Ingeniera
	glm::vec3(0.0f, 3.5f,   0.0f),  // centro
	glm::vec3(15.0f, 3.5f, -16.0f)   // cerca de Hombre
};
const int NUM_LAMPS = sizeof(lampPositions) / sizeof(lampPositions[0]);

bool lampsOn = false;  // toggle de lamparas rojas (tecla L)
...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// Point light 1 (toggle con SPACE)
		glm::vec3 lightColor;
		...
		// Lamparas rojas (toggle con L) - usan pointLights[1] hasta pointLights[NUM_LAMPS]
		glm::vec3 lampColor = lampsOn ? glm::vec3(1.0f, 0.0f, 0.0f) : glm::vec3(0.0f);

		for (int i = 0; i < NUM_LAMPS; i++)
		{
			std::string idx = std::to_string(i + 1);  // +1 porque pointLights[0] ya lo usa SPACE
...
void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mode)
{
	...
	// Toggle lamparas rojas con tecla L
